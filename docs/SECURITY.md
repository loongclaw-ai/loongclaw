...
- Direct app tool execution for `shell.exec`, `file.read`, `file.write`, `file.edit`, `claw.migrate`, and memory-corpus file tools now reuses the same shared shell/file governance helpers before dispatch; execution-layer command/path validation remains in place as defense in depth
