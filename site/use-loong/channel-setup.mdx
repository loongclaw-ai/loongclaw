...
- when authoring a managed bridge plugin, keep `channel_runtime_operation_specs_json` aligned with each declared runtime operation so Loong can surface labels, summaries, and sample payload hints instead of only raw operation ids
