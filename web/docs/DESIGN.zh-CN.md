...
> 最后更新：2026-03-22（已补专项 review 修复）
...
## 9. 专项 review 修复结果（2026-03-22）

这轮 WebUI 专项 review 先集中处理了真实运行时问题，而不是继续叠加新功能。已经落地并验证通过的修复如下：

### 9.1 已修复：Chat 流式失败不再误删已接受消息


### 9.2 已修复：发送失败会恢复输入框内容


### 9.3 已修复：Dashboard 部分成功保存会回拉真实状态


### 9.4 已修复：onboarding 不再把鉴权失败误判成 runtime offline


## 10. 当前仍值得优先关注的事项

专项 review 之后，最值得继续跟进的是：

1. **流式中断与提前关闭语义**
   - `streamTurn` 仍值得补更明确的中断控制
   - 提前关闭时的 `turn.failed` 降级仍可继续强化
2. **大页面继续温和拆分**
   - `ChatPage`
   - `DashboardPage`
   - `OnboardingStatusPanel`
   仍然偏大，后续开发应继续优先抽 hook / 子组件
3. **Chat 页 i18n 清理**
   - 仍有少量硬编码英文字符串需要补齐到 locale key
4. **Debug Console 继续从“可用原型”往“连续只读输出流”推进**
5. **安装态与同源产品态继续收口**
   - 当前第一版骨架已具备
   - 后续重点会转向更顺的安装体验与产品态文案/状态收口
