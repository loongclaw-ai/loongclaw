...
pub async fn run_acp_close_cli(
    ...
    if as_json {
        let payload = json!({
            ...
            "requested_session": execution.requested_session_key,
...
pub fn resolve_acp_status_session_key(
    ...
    resolve_acp_session_key_for_command(
        config,
        session_key,
        conversation_id,
        route_session_id,
        RUNTIME_ACP_STATUS_COMMAND,
    )
}

fn resolve_acp_session_key_for_command(
    config: &mvp::config::LoongConfig,
    session_key: Option<&str>,
    conversation_id: Option<&str>,
    route_session_id: Option<&str>,
    ...
    match (session_key, conversation_id, route_session_id) {
        ...
        (Some(_), Some(_), _) | (Some(_), _, Some(_)) | (_, Some(_), Some(_)) => Err(format!(
            "{command_name} accepts exactly one of --session, --conversation-id, or --route-session-id"
        )),
        (None, None, None) => Err(format!(
            "{command_name} requires --session <session_key>, --conversation-id <conversation_id>, or --route-session-id <route_session_id>"
        )),
    }
}
