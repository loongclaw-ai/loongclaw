...
use crate::tools::{
    ToolView, delegate_child_tool_view_for_config, delegate_child_tool_view_for_contract,
};
            ...
            if let Ok(repo) = SessionRepository::new(&memory_config) {
                    ...
                    .map_err(|error| format!("load session context failed: {error}"))?
                {
                    if let Some(parent_session_id) = session.parent_session_id {
                        ...
                        let session_context = match session.label {
                            Some(label) => session_context.with_subagent_identity(
                                ConstrainedSubagentIdentity {
                                    nickname: Some(label),
                                    specialization: None,
                                },
                            ),
                        ...
                        let session_context = match execution {
                            Some(execution) => session_context.with_subagent_execution(execution),
                            None => match derive_subagent_profile_from_lineage(
                                &repo,
                                session_id,
                                config.tools.delegate.max_depth,
                            )? {
                                Some(subagent_profile) => {
                                    session_context.with_subagent_profile(subagent_profile)
                                }
                                None => session_context,
                            },
                        };
                    ...
                    && let Some(parent_session_id) = summary.parent_session_id
                {
                    ...
                    let session_context = match summary.label {
                        Some(label) => {
            ...
            if let Ok(repo) = SessionRepository::new(&memory_config) {
                    ...
                    .map_err(|error| format!("load session tool-view context failed: {error}"))?
                {
                    if session.parent_session_id.is_some() {
                        let subagent_contract = match load_delegate_execution(&repo, session_id)? {
                            Some(execution) => Some(execution.contract_view()),
