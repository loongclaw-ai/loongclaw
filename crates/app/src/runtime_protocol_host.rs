...
pub struct LoongAppRuntimeProtocolHost<'a> {
    ...
    loaded_config: Option<crate::config::LoongConfig>,
}
...
impl<'a> LoongAppRuntimeProtocolHost<'a> {
    ...
    pub fn with_loaded_config(mut self, loaded_config: crate::config::LoongConfig) -> Self {
...
impl AppProtocolRuntimeHost for LoongAppRuntimeProtocolHost<'_> {
        ...
        let result = turn_service
            .execute(
                ...
                TurnExecutionOptions {
                    ...
                    observer: None,
                    ingress: None,
                    provenance: crate::acp::AcpTurnProvenance::default(),
    ...
    async fn resolve_latest_root_session_id(
        ...
        #[cfg(feature = "memory-sqlite")]
        {
            let loaded = load_runtime_config(config, self.loaded_config.as_ref())?;
            let memory_config =
                crate::session::store::SessionStoreConfig::from_memory_config(&loaded.memory);
            return crate::session::latest_resumable_root_session_id(&memory_config);
        }
...
fn load_runtime_config(
    config: &RuntimeExecutorConfig,
    loaded_override: Option<&crate::config::LoongConfig>,
        ...
        .map_err(|error| format!("failed to access config path {}: {error}", path.display()))?
    {
        ...
        let (_resolved_path, config) = crate::config::load(Some(path_string.as_str()))?;
        return Ok(config);
    }
...
fn latest_root_session_id(config: &RuntimeExecutorConfig) -> CliResult<Option<String>> {
    #[cfg(feature = "memory-sqlite")]
    {
        ...
        let memory_config =
            crate::session::store::SessionStoreConfig::from_memory_config(&loaded.memory);
        crate::session::latest_resumable_root_session_id(&memory_config)
    }
