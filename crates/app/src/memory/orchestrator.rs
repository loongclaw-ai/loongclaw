...
impl BuiltinMemoryOrchestrator {
    pub fn hydrate_stage_envelope(
        ...
        let retrieval_request = Some(build_builtin_retrieval_request(session_id, config));
    ...
    pub fn hydrate(
        ...
        Ok(self.hydrate_stage_envelope(session_id, config)?.hydrated)
    }
}
...
pub fn hydrate_stage_envelope(
    ...
    match config.system {
        MemorySystemKind::Builtin => {
            BuiltinMemoryOrchestrator.hydrate_stage_envelope(session_id, config)
        }
    }
}
